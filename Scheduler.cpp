 ...
 * ALGORITHM OVERVIEW:
 * ------------------
 * This scheduler implements a greedy best-fit consolidation strategy to minimize
 * energy consumption while respecting SLA constraints. The key principles are:
 * 
 * 1. CONSOLIDATION: Pack tasks onto the fewest active servers to maximize
 *    utilization and allow idle servers to sleep.
 * 
 * 2. ENERGY-AWARE PLACEMENT: For each task, evaluate all feasible servers and
 *    select the one with lowest incremental power cost, computed as:
 *      - Wake penalty (if server is sleeping)
 *      - Base power (if transitioning from off to active)
 *      - Marginal dynamic power based on utilization increase
 * 
 * 3. SLA PRIORITIZATION: Tasks with tighter SLAs (SLA0 > SLA1 > SLA2 > SLA3)
 *    receive higher scheduling priority and are placed first.
 * 
 * 4. POWER MANAGEMENT: Servers idle for IDLE_COOLDOWN_TICKS are transitioned
 *    to low-power state (S5), with at least one server per CPU type kept warm.
 * 
 * CONFIGURATION PARAMETERS:
 * ------------------------
 * Tune these constants at the top of the file for different workload characteristics:
 * 
 * - CONSOLIDATE_FIRST (default: true)
 *   When true, only considers active servers first. Set false for workloads
 *   with strict latency requirements where wake delays are unacceptable.
 * 
 * - IDLE_COOLDOWN_TICKS (default: 5)
 *   Number of periodic checks a server must be idle before sleeping.
 *   Increase for bursty workloads to avoid thrashing. Decrease for steady
 *   workloads to save more energy.
 * 
 * - WAKE_PENALTY_WATTS (default: 20.0)
 *   Estimated energy cost of waking a server. Adjust based on measured
 *   transition costs from your hardware.
 * 
 * - UTIL_POWER_SLOPE (default: 0.6)
 *   Linear approximation of utilization-to-power relationship.
 *   0.0 = constant power, 1.0 = perfectly linear. Tune based on your
 *   server's actual power curve.
 * 
 * - ENABLE_SCHEDULER_LOGGING (default: 1)
 *   Set to 0 to disable detailed logging for production runs.
 * 
 * MEASUREMENT AND EVALUATION:
 * --------------------------
 * The scheduler logs key metrics for analysis:
 * 
 * 1. ENERGY METRICS:
 *    - Total cluster energy (KW-Hour) - primary optimization target
 *    - Per-tick estimated power consumption
 *    - Active machine count over time
 * 
 * 2. PERFORMANCE METRICS:
 *    - SLA violation rates (SLA0, SLA1, SLA2)
 *    - Task placement decisions and power costs
 *    - Pending task queue depth
 * 
 * 3. OPERATIONAL METRICS:
 *    - Server wake/sleep events
 *    - Utilization levels per server
 *    - Idle tick counts
 * 
 * CREATING TEST WORKLOADS:
 * -----------------------
 * Design input files to stress different aspects of the scheduler:
 * 
 * 1. SURGE WORKLOAD:
 *    - Sudden spike in task arrivals
 *    - Tests wake-up latency and consolidation under load
 *    - Expect: More machines activated, higher energy but maintained SLA
 * 
 * 2. LOW-INTENSITY WORKLOAD:
 *    - Sparse, steady task arrivals
 *    - Tests aggressive consolidation and sleep behavior
 *    - Expect: Minimal active machines, lowest energy, good SLA
 * 
 * 3. HETEROGENEOUS WORKLOAD:
 *    - Mix of CPU types (X86, ARM, POWER, RISCV)
 *    - Mix of GPU-capable and CPU-only tasks
 *    - Tests type-aware placement and multi-type consolidation
 *    - Expect: Type-specific clustering, balanced wake penalties
 * 
 * 4. SLA-STRESSED WORKLOAD:
 *    - Many SLA0/SLA1 tasks with tight deadlines
 *    - Tests priority handling and placement speed
 *    - Expect: May sacrifice some energy for SLA compliance
 * 
 * 5. MEMORY-CONSTRAINED WORKLOAD:
 *    - Tasks with high memory requirements
 *    - Tests fit checking and fragmentation handling
 *    - Expect: More machines needed, potential pending queue buildup
 * 
 * EXPERIMENTAL COMPARISONS:
 * ------------------------
 * Compare variants by adjusting config flags:
 * 
 * - Baseline: CONSOLIDATE_FIRST=true, IDLE_COOLDOWN=5
 * - Aggressive: CONSOLIDATE_FIRST=true, IDLE_COOLDOWN=2
 * - Conservative: CONSOLIDATE_FIRST=true, IDLE_COOLDOWN=10
 * - Latency-first: CONSOLIDATE_FIRST=false, IDLE_COOLDOWN=5
 * 
 * For each variant, measure:
 * - Total energy consumption
 * - SLA violation rates
 * - Average response time
 * - Peak active machines
 * 
 * LIMITATIONS AND FUTURE WORK:
 * ---------------------------
 * Current implementation is deterministic and greedy. Potential enhancements:
 * 
 * - VM migration for dynamic load balancing
 * - Predictive wake-up based on arrival patterns
 * - Multi-objective optimization (energy vs. latency Pareto frontier)
 * - Per-core DVFS (P-state) tuning for fine-grained power control
 * - Heterogeneous-aware bin packing for better type utilization
 * 
 * TODO MARKERS FOR SIMULATOR INTEGRATION:
 * ---------------------------------------
 * All simulator-specific APIs are already mapped via Interfaces.h.
 * If your simulator uses different names or signatures, search for these
 * function calls and adjust:
 * 
 * - Machine_GetInfo(), Machine_GetTotal(), Machine_SetState()
 * - VM_Create(), VM_Attach(), VM_AddTask(), VM_GetInfo(), VM_Shutdown()
 * - GetTaskInfo(), RequiredCPUType(), RequiredVMType(), GetTaskMemory()
 * - IsTaskGPUCapable(), RequiredSLA(), GetTaskPriority()
 * - Now(), Machine_GetClusterEnergy(), GetSLAReport()
 * 
 * The algorithm logic remains unchanged regardless of API naming.
 */
